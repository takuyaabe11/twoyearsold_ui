/// TWOYEARSOLD 共有デザイン基盤(One World)。
///
/// 全ゲームが共有する「土台」: 役割トークン/テーマ・モーション・ブランド表記・
/// 共通ウィジェット・設定(テーマ/言語/効果音/演出)。仕様は ../DESIGN.md §10。
library;

export 'src/app_theme.dart';
export 'src/motion.dart';
export 'src/back_button.dart';
export 'src/bootstrap.dart';
export 'src/brand_footer.dart';
export 'src/choice_row.dart';
export 'src/footer_link.dart';
export 'src/editorial_scaffold.dart';
export 'src/formatters.dart';
export 'src/game_clock.dart';
export 'src/game_header.dart';
export 'src/game_over_overlay.dart';
export 'src/haptics.dart';
export 'src/headings.dart';
export 'src/json_slot_repository.dart';
export 'src/help_button.dart';
export 'src/menu_row.dart';
export 'src/language_pulldown.dart';
export 'src/primary_action.dart';
export 'src/rules_page.dart';
export 'src/section_label.dart';
export 'src/safe_change_notifier.dart';
export 'src/sound_service.dart';
export 'src/star_picker.dart';
export 'src/stat_tile.dart';
export 'src/theme_name.dart';
export 'src/twoyearsold_app.dart';
export 'src/l10n.dart';
export 'src/providers.dart';
export 'src/settings_controller.dart';
export 'src/settings_screen.dart';
